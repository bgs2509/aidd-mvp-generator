...
**Решение**: Команда `/init` автоматически копирует файлы команд:

```bash
# При /init выполняется:
mkdir -p .claude/commands
cp .aidd/.claude/commands/*.md .claude/commands/
```

После `/init` команды доступны в автодополнении CLI:
```
/idea, /research, /plan, /feature-plan, /generate,
/review, /test, /validate, /deploy
```

**Обновление команд**: При обновлении submodule (`.aidd/`) повторно запустите `/init` —
...
Пользователь: /idea "описание"
     ↓
Claude Code загружает: ./.claude/commands/idea.md (копия из .aidd/)
     ↓
...
### Режимы инициализации (`/init`)

Команда `/init` автоматически определяет тип проекта и запускает соответствующий режим:
