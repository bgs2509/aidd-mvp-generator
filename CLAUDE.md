# CLAUDE.md — Главная точка входа для AI-агентов

> **Назначение**: Этот файл — первый документ, который AI-агент должен прочитать.
> Содержит обзор фреймворка, правила работы и порядок чтения документации.
>
> **Философия**: VERIFY BEFORE ACT — Проверяй перед действием.

---

## Обзор фреймворка AIDD-MVP Generator

**AIDD-MVP Generator** — фреймворк для быстрой генерации production-ready MVP проектов,
объединяющий методологию AI-Driven Development (AIDD) с архитектурными шаблонами.

### Ключевые характеристики

| Параметр | Значение |
|----------|----------|
| Уровень зрелости | **Level 2 (MVP)** — всегда |
| Покрытие тестами | ≥75% |
| Время генерации | ~10 минут |
| Архитектура | DDD/Hexagonal, HTTP-only доступ к данным |
| Качественные ворота | 8 обязательных gates |

### Два режима работы

| Режим | Команда | Описание |
|-------|---------|----------|
| **CREATE** | `/idea "описание"` | Создание нового MVP проекта с нуля |
| **FEATURE** | `/idea "описание фичи"` | Добавление функционала в существующий проект |

---

## Интеграция фреймворка в проект

> **КЛЮЧЕВАЯ ИДЕЯ**: Фреймворк подключается в целевой проект как Git Submodule.
> AI-агент читает инструкции из submodule и генерирует код в целевом проекте.

### Два способа использования

| Способ | Когда использовать | Пути к файлам |
|--------|-------------------|---------------|
| **Submodule** | Работа над реальным проектом | `.aidd/docs/...` |
| **Direct** | Разработка самого фреймворка | `docs/...` |

### Установка фреймворка (Submodule)

```bash
# 1. Создать целевой проект
mkdir my-mvp-project && cd my-mvp-project
git init

# 2. Подключить фреймворк как submodule
git submodule add https://github.com/your-org/aidd-mvp-generator.git .aidd
git submodule update --init --recursive

# 3. Создать точку входа для AI
echo "Читай .aidd/CLAUDE.md" > CLAUDE.md

# 4. Запустить Claude Code
claude
/idea "Описание проекта"
```

### Структура целевого проекта после интеграции

```
my-mvp-project/                      ← ЦЕЛЕВОЙ ПРОЕКТ
│
├── .aidd/                           ← Git Submodule (НЕ МОДИФИЦИРОВАТЬ!)
│   ├── CLAUDE.md                    ← Точка входа для AI
│   ├── workflow.md
│   ├── conventions.md
│   ├── .claude/agents/              ← Роли AI
│   ├── .claude/commands/            ← Slash-команды
│   ├── templates/                   ← Шаблоны сервисов
│   ├── knowledge/                   ← База знаний
│   └── docs/templates/              ← Шаблоны документов
│
├── CLAUDE.md                        ← Точка входа (ссылается на .aidd/)
├── .pipeline-state.json             ← Состояние пайплайна
│
├── ai-docs/docs/                    ← AI-ГЕНЕРИРУЕМЫЕ ДОКУМЕНТЫ
│   ├── prd/{name}-prd.md
│   ├── architecture/{name}-plan.md
│   ├── plans/{feature}-plan.md
│   ├── reports/
│   └── rtm.md
│
├── services/                        ← AI-ГЕНЕРИРУЕМЫЙ КОД
│   ├── {context}_{domain}_api/
│   ├── {context}_{domain}_data/
│   └── ...
│
└── docker-compose.yml               ← AI-ГЕНЕРИРУЕМАЯ ИНФРАСТРУКТУРА
```

### Критическое правило

```
┌─────────────────────────────────────────────────────────────────┐
│  ⛔ НИКОГДА НЕ МОДИФИЦИРОВАТЬ ФАЙЛЫ В .aidd/                     │
├─────────────────────────────────────────────────────────────────┤
│  • Фреймворк = Knowledge Base (только чтение)                   │
│  • Весь код генерируется ВОВНЕ submodule                        │
│  • Для обновления: git submodule update --remote                │
└─────────────────────────────────────────────────────────────────┘
```

### Обновление фреймворка

```bash
# Получить последнюю версию фреймворка
cd my-mvp-project
git submodule update --remote .aidd
git add .aidd
git commit -m "chore: обновление aidd-mvp-generator"
```

---

## Порядок чтения документации

AI-агент ДОЛЖЕН читать документы в следующем порядке:

```
1. CLAUDE.md (этот файл)     ← Обзор и правила
        │
        ▼
2. docs/INDEX.md             ← Индекс всех файлов генератора
        │
        ▼
3. conventions.md            ← Соглашения о коде
        │
        ▼
4. workflow.md               ← Процесс разработки (8 этапов)
        │
        ▼
5. docs/NAVIGATION.md        ← Матрица "читать → создавать" по этапам
        │
        ▼
6. .claude/agents/{role}.md  ← Инструкции для текущей роли
        │
        ▼
7. roles/{role}/*.md         ← Детальные инструкции функций
        │
        ▼
8. knowledge/{topic}/        ← База знаний по теме
```

**Быстрый поиск**: Если нужно найти файл — см. [docs/INDEX.md](docs/INDEX.md)

---

## Структура фреймворка

```
aidd-mvp-generator/
│
├── CLAUDE.md              ← ВЫ ЗДЕСЬ — точка входа
├── conventions.md         ← Соглашения о коде и стиле
├── workflow.md            ← Описание 8-этапного процесса
├── README.md              ← Общая документация проекта
│
├── .claude/               ← Интеграция Claude Code
│   ├── settings.json      ← Permissions и hooks
│   ├── agents/            ← Subagents для Task tool (7 ролей)
│   │   ├── analyst.md
│   │   ├── researcher.md
│   │   ├── architect.md
│   │   ├── implementer.md
│   │   ├── reviewer.md
│   │   ├── qa.md
│   │   └── validator.md
│   │
│   └── commands/          ← 9 slash-команд
│       ├── idea.md        → /idea
│       ├── research.md    → /research
│       ├── plan.md        → /plan
│       ├── feature-plan.md → /feature-plan
│       ├── generate.md    → /generate
│       ├── review.md      → /review
│       ├── test.md        → /test
│       ├── validate.md    → /validate
│       └── deploy.md      → /deploy
│
├── roles/                 ← Детальные инструкции по ролям
│   ├── analyst/           ← Аналитик: PRD, требования
│   ├── researcher/        ← Исследователь: анализ кода
│   ├── architect/         ← Архитектор: проектирование
│   ├── implementer/       ← Реализатор: генерация кода
│   ├── reviewer/          ← Ревьюер: код-ревью
│   ├── qa/                ← QA: тестирование
│   └── validator/         ← Валидатор: проверка ворот
│
├── knowledge/             ← База знаний
│   ├── architecture/      ← Архитектурные принципы
│   ├── services/          ← FastAPI, Aiogram, Workers
│   ├── integrations/      ← HTTP, Redis
│   ├── infrastructure/    ← Docker, Nginx, CI/CD
│   └── quality/           ← Тестирование, логирование
│
├── templates/             ← Шаблоны для генерации
│   ├── services/          ← Шаблоны 5 типов сервисов
│   ├── shared/            ← Общие компоненты
│   └── infrastructure/    ← Docker, Nginx, GitHub Actions
│
└── docs/                  ← Документация ГЕНЕРАТОРА
    ├── templates/         ← Шаблоны документов для целевого проекта
    │   ├── prd-template.md
    │   └── architecture-template.md
    ├── target-project-structure.md  ← Структура целевого проекта
    └── history/           ← История изменений генератора
```

---

## Генератор vs Целевой проект

> **КРИТИЧЕСКИ ВАЖНО**: Это ДВА РАЗНЫХ проекта!
> Генератор подключается к целевому проекту как Git Submodule (см. [Интеграция](#интеграция-фреймворка-в-проект)).

| Аспект | Генератор (.aidd/) | Целевой проект (корень) |
|--------|-------------------|------------------------|
| **Тип** | Git Submodule | Основной репозиторий |
| **Назначение** | Инструкции, шаблоны | Приложение, код |
| **Модификация** | ⛔ ЗАПРЕЩЕНО | ✅ Разрешено |
| **Содержит** | Knowledge base, templates | Сервисы, артефакты |
| **Артефакты** | `.aidd/docs/templates/*.md` | `ai-docs/docs/*.md` |
| **Обновление** | `git submodule update` | `git commit` |

### Пути артефактов (ТОЛЬКО в целевом проекте!)

```
{project-name}/                    ← Целевой проект (НЕ в .aidd/)
├── ai-docs/docs/
│   ├── prd/{name}-prd.md         ← PRD документ
│   ├── architecture/{name}-plan.md  ← План архитектуры
│   ├── plans/{feature}-plan.md   ← План фичи
│   ├── reports/                  ← Отчёты
│   └── rtm.md                    ← RTM
├── services/                     ← Код сервисов
└── .pipeline-state.json          ← Состояние пайплайна
```

**Подробнее**: [docs/target-project-structure.md](docs/target-project-structure.md)

---

## Критические правила

### 1. Архитектурные принципы

| Принцип | Правило |
|---------|---------|
| **HTTP-only** | Бизнес-сервисы НИКОГДА не обращаются к БД напрямую |
| **DDD/Hexagonal** | Разделение на api/application/domain/infrastructure |
| **Async-First** | Все I/O операции используют async/await |
| **Типобезопасность** | Полные type hints для всех функций |

### 2. Правила верификации (VERIFY BEFORE ACT)

```
┌─────────────────────────────────────────────────────────────────┐
│  ПЕРЕД ЛЮБЫМ ДЕЙСТВИЕМ AI ОБЯЗАН:                               │
├─────────────────────────────────────────────────────────────────┤
│  1. СОЗДАНИЕ ФАЙЛА  → Проверить, что файл НЕ существует         │
│  2. РЕДАКТИРОВАНИЕ  → Сначала прочитать текущее содержимое      │
│  3. УДАЛЕНИЕ        → Проверить все зависимости и ссылки        │
│  4. ДОБАВЛЕНИЕ ССЫЛКИ → Проверить, что цель существует          │
│  5. НАПИСАНИЕ КОДА  → Проверить нет ли похожего кода (DRY)      │
│  6. ДОБАВЛЕНИЕ ФИЧИ → Проверить, что это нужно СЕЙЧАС (YAGNI)   │
└─────────────────────────────────────────────────────────────────┘

НИКОГДА НЕ ПРЕДПОЛАГАТЬ → ВСЕГДА ПРОВЕРЯТЬ → ЗАТЕМ ДЕЙСТВОВАТЬ
```

### 3. Качественные ворота

> **Примечание**: Качественные ворота — это концепция фреймворка, а НЕ встроенная функция Claude Code.
> Проверка ворот реализуется через инструкции в промптах команд.

AI-агент НЕ МОЖЕТ перейти к следующему этапу без прохождения ворот:

| Этап | Ворота | Критерии |
|------|--------|----------|
| 1. Идея | `PRD_READY` | PRD заполнен, нет блокирующих вопросов |
| 2. Исследование | `RESEARCH_DONE` | Код проанализирован, паттерны выявлены |
| 3. Архитектура | `PLAN_APPROVED` | План утверждён пользователем |
| 4. Реализация | `IMPLEMENT_OK` | Код написан, тесты проходят |
| 5. Ревью | `REVIEW_OK` | Код соответствует стандартам |
| 6. QA | `QA_PASSED` | Coverage ≥75%, нет критических багов |
| 7. Валидация | `ALL_GATES_PASSED` | Все артефакты проверены |
| 8. Деплой | `DEPLOYED` | Приложение запущено |

---

## Типы сервисов

Фреймворк поддерживает генерацию 5 типов сервисов:

| Тип | Шаблон | Порт | Описание |
|-----|--------|------|----------|
| **Business API** | `fastapi_business_api/` | 8000-8099 | REST API на FastAPI |
| **Business Bot** | `aiogram_bot/` | — | Telegram бот на Aiogram |
| **Background Worker** | `asyncio_worker/` | — | Фоновые задачи на AsyncIO |
| **Data API PostgreSQL** | `postgres_data_api/` | 8001 | CRUD для PostgreSQL |
| **Data API MongoDB** | `mongo_data_api/` | 8002 | CRUD для MongoDB |

---

## Именование

### Сервисы
```
{контекст}_{домен}_{тип}

Примеры:
- finance_lending_api      — API для кредитования
- finance_lending_bot      — Telegram бот для кредитования
- finance_lending_worker   — Фоновый воркер
- finance_lending_data     — Data API (PostgreSQL)
```

### Файлы
```
- snake_case для Python файлов: user_service.py
- kebab-case для конфигов: docker-compose.yml
- Docstrings на русском языке в Google-стиле
```

---

## Быстрый старт

### Вариант 1: Новый проект с фреймворком (рекомендуется)

```bash
# 1. Создать и инициализировать проект
mkdir restaurant-booking && cd restaurant-booking
git init

# 2. Подключить фреймворк
git submodule add https://github.com/your-org/aidd-mvp-generator.git .aidd

# 3. Создать точку входа
echo "# Restaurant Booking\n\nСм. .aidd/CLAUDE.md для работы с AI." > CLAUDE.md

# 4. Запустить Claude Code и начать работу
claude
/idea "Создать сервис бронирования столиков в ресторанах"

# 5. Следовать 8-этапному процессу
# /idea → /research → /plan → /generate → /review → /test → /validate → /deploy
```

### Вариант 2: Добавление фичи в существующий проект

```bash
# 1. Перейти в директорию проекта (где уже есть .aidd/)
cd my-existing-project

# 2. Запустить Claude Code
claude

# 3. Описать фичу
/idea "Добавить систему уведомлений по email"

# 4. Следовать процессу для фичи
# /idea → /research → /feature-plan → /generate → /review → /test → /validate → /deploy
```

### Вариант 3: Разработка самого фреймворка (для контрибьюторов)

```bash
# 1. Клонировать репозиторий фреймворка напрямую
git clone https://github.com/your-org/aidd-mvp-generator.git
cd aidd-mvp-generator

# 2. Работать с файлами фреймворка
# (этот режим — для разработки самого фреймворка, НЕ для создания проектов)
```

---

## Конфигурация Claude Code

### settings.json

Файл `.claude/settings.json` конфигурирует поведение Claude Code:

```json
{
  "permissions": {
    "allow": ["Read(**)", "Glob(**)", ...],  // Автоматически разрешённые действия
    "deny": ["Edit(**/.env)", ...]            // Запрещённые действия
  },
  "hooks": {
    "PreToolUse": [...],   // Хуки перед использованием инструмента
    "PostToolUse": [...]   // Хуки после использования инструмента
  }
}
```

**Назначение permissions**:
- `allow`: Действия, которые Claude Code выполняет без подтверждения
- `deny`: Действия, которые заблокированы (защита secrets, опасные команды)

### Разграничение tools

| Где определено | Что значит |
|----------------|------------|
| `.claude/agents/*.md` → `tools:` | Инструменты для Task tool (subagents) |
| `.claude/commands/*.md` → `allowed-tools:` | Разрешения для конкретной команды |
| `.claude/settings.json` → `permissions` | Глобальные разрешения проекта |

---

## Политика .ai-framework/

> **Статус**: Справочный материал (read-only)

### Назначение

Директория `.ai-framework/` содержит альтернативную реализацию AIDD фреймворка.
Этот код используется как **справочник** для заимствования идей.

### Правила использования

| Действие | Разрешено | Примечание |
|----------|-----------|------------|
| Чтение документации | ✓ Да | Для изучения подходов |
| Копирование идей | ✓ Да | Адаптировать под 8-этапный процесс |
| Редактирование | ✗ Нет | Оставить как есть |
| Удаление | ✗ Нет | Сохранить для истории |
| Интеграция как submodule | ✗ Нет | НЕ использовать git submodule |

### Почему НЕ submodule (P-035)

`.ai-framework/` **НЕ ДОЛЖЕН** быть git submodule:
1. Это архивный справочник, не активная зависимость
2. Submodule создаёт сложности с синхронизацией
3. Основной фреймворк — `aidd-mvp-generator/`, не `.ai-framework/`

### Что можно заимствовать

- AI Navigation Matrix → адаптировать для 8 этапов
- Шаблоны документов → если отсутствуют в `docs/templates/`
- Архитектурные идеи → переносить в `knowledge/`

### Что НЕ заимствовать

- 7-этапный процесс (у нас 8 этапов)
- Пути артефактов (у нас `ai-docs/docs/`)
- Структуру агентов (у нас `.claude/agents/`)

---

## Связанные документы

| Документ | Описание |
|----------|----------|
| [conventions.md](conventions.md) | Соглашения о коде и стиле |
| [workflow.md](workflow.md) | 8-этапный процесс разработки |
| [.claude/agents/](.claude/agents/) | Определения AI-ролей |
| [knowledge/architecture/](knowledge/architecture/) | Архитектурные принципы |
| [docs/INDEX.md](docs/INDEX.md) | Индекс файлов генератора |
| [docs/NAVIGATION.md](docs/NAVIGATION.md) | Навигационная матрица |
| [docs/target-project-structure.md](docs/target-project-structure.md) | Структура целевого проекта |

---

**Версия документа**: 1.1
**Создан**: 2025-12-19
**Назначение**: Главная точка входа для AI-агентов AIDD-MVP Generator
