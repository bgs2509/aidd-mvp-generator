...
Принципы качества проверялись ТОЛЬКО на этапе Validate.
Ошибки DRY/KISS/YAGNI проходили через весь пайплайн.
```

**Пример**: На этапе `/aidd-research` предлагается структура из 5 файлов с нарушениями DRY.
Ошибка обнаруживается только на Validate — после генерации кода.
...
| `.claude/agents/planner.md` | 16 проверок для Architect |
| `.claude/agents/coder.md` | 17 проверок для Implementer |
